...
<body>
    ...
    <div class="loginCard">
        <div style="text-align: right;">
            <a href="edit_coach_manage.php" style="font-size: 36px; text-decoration: none;color:#17150e">&times;</a>
        </div>
        <form id="add-coach-form" action="add_coach.php" method="post">
    ...
    <script>
        $(document).ready(function() {
            $('#add-coach-form').on('submit', function(e) {
                e.preventDefault();
                $.ajax({
                    url: 'add_coach.php',
                    type: 'POST',
                    ...
                    success: function(response) {
                        if (response.success) {
                            $('#response-message').html('<div class="alert alert-success">教练添加成功！</div>');
                            setTimeout(function() {
                                window.location.href = 'edit_coach_manage.php';
                            }, 2000); // 2秒后重定向
                        } else {
