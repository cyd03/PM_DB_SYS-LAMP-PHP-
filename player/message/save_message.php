...
function update_message($mysqli, $name, $sex, $email, $tel, $u_name,$height,$weight,$id)
{
    // 开启事务
    ...
    try {
        ...
        $stmt3->bind_param('iii', $height, $weight, $id);
        if (!$stmt3->execute()) {
...
try {
    update_message($mysqli, $name, $sex, $email, $tel, $u_name,$height,$weight,$id);
    // 更新成功，重定向回个人信息页面
    $mysqli->close();
    exit;
} catch (Exception $e) {
